...
/**
 * Creates a new node.  For this use case, we use 'unsigned' for the data type.
 */
HashNode* new_node(char* key, unsigned int data) {
 ...
 */
void ht_set(HashTable* ht, char* key, unsigned value) {
 ...
 * Since we are storing unsigned values, we return 0 if
